...
<div class="cms-product-info-form">
    ...
    if (!$model->isNewRecord && !empty($model->product_mask)) {
        $pluginOptions['initialPreview'] = \Yii::getAlias('@web').$model->product_mask;
        $pluginOptions['initialPreviewAsData'] = true;
    }
    ?>
    <?= $form->field($model, 'product_mask_file')->widget(FileInput::classname(), ['options' => ['accept' => 'image/jpg,image/jpeg,image/png'],
        'pluginOptions' => $pluginOptions
    ])->hint(Yii::t('app', 'Required').', '.Yii::t('app', 'File type: ').'jpg\png,'.Yii::t('app', 'File max size: ').' 2MB')->label('悬浮图');?>
