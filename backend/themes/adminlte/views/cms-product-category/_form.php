...
<div class="cms-product-category-form">
    ...
    <?= $form->field($model, 'parent_id')->dropDownList(DataHelper::getProductType(),['prompt'=>Yii::t('app', 'Please select'),'style'=>'width:20%;'])->hint(Yii::t('app', 'This item\'s parent node.')) ?>
    ...
    <?= $form->field($model, 'image_node_file')->widget(FileInput::classname(), ['options' => ['accept' => 'image/jpg,image/jpeg,image/png'],
        'pluginOptions' => $pluginOptions
    ])->hint(Yii::t('app', 'File type: ').'jpg\png,'.Yii::t('app', 'File max size: ').' 2MB')->label('app下载');?>
    
    <?= $form->field($model, 'src')->textInput(['style'=>'width:80%;'])->hint('banner观看电影链接'); ?>
