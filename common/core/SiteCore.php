...
class SiteCore {
    ...
    function init() {
        ...
        $paramLangId = isset(\Yii::$app->session['lang_id'])?\Yii::$app->session['lang_id']:1;
        ...
        $site = [];
         if ($hostName == "localhost" || $hostName == "127.0.0.1" || $hostName == "demo.bothsite.com"||$hostName == "o.gohoc.com") {
        	...
        	if (empty($site)) {
        		//不存在对应的站点
        		InitHelper::initDefalutData($paramSiteId,'http://'.$hostName,$paramLangId);
        	}
        	$site = $this->loadGhSiteByHostName('http://'.$hostName); 
        }              
        //初始化theme
        ...
        if($newsBanner = CmsBanner::find()->with('images')->where(['site_id'=>$siteId,'lang_id'=>$lang_id])->andWhere(['like','pos',',homefooter,'])->orderBy('sort_val asc')->asArray()->one()){
        	$siteInfo['news-banner']=$newsBanner;
        }
    ...
    function loadCmsSiteInfo($site_id,$lang_id=1) {
        $cmsSite = CmsSite::find()->with(['contact'])->with([
        		'articles' => function($query) {
        			$cate=CmsCategory::find()->where(['type'=>CmsCategory::CATE_QUESTION])->one();
			        $query->where(['category_id'=>$cate->id])->limit(2)->orderby('created_at desc');
			    },
        ])->where(['site_id' => $site_id,'lang_id'=>$lang_id])->asArray()->one();
        return $cmsSite;
    }
