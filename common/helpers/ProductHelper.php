...
class ProductHelper
{
    ...
    {
        $categoryOptions = CacheHelper::getCache('product_cate_'.$site_id.'_'.$lang_id);
        $data='';
        if (empty($categoryOptions))
        {
            $categoryOptions = CmsProductCategory::find()->where(['site_id'=>$site_id,'lang_id'=>$lang_id])->asArray()->all();
            //var_dump($categoryOptions);die();
            $data=[];
            if (!empty($categoryOptions)){
            	foreach ($categoryOptions as $val){
            		$data[$val['id']]=$val['name'];
            	}
            }
            CacheHelper::setCache('product_cate_'.$site_id.'_'.$lang_id,$data);
        }
        return $data;
    }
    ...
    static public function getProductCache($product_id)
    {
        ...
        if (empty($product))
        {
            ...
            $product['files']=CmsUploadFile::find()->where(['product_id'=>$product_id])->asArray()->all();
